 ...
 * S3FileStorage
 *
 * FileStorageInterface implementation backed by Amazon S3. Server-side
 * encryption is always on: SSE-KMS when AWS_S3_KMS_KEY_ID is configured,
 * otherwise SSE-S3 (AES256) by default — PHI at rest is never written
 * unencrypted regardless of admin configuration.
 *
...
final class S3FileStorage implements FileStorageInterface
{
    ...
    public function downloadToPath(string $key, string $destinationPath, ?string $versionId = null): void
    {
        ...
        // The AWS SDK's GetObject response Body is a Psr7 stream object
        // (GuzzleHttp\Psr7\Stream implementing StreamInterface) in normal
        // operation — not a raw resource and not a string. Both of those
        // remain supported as defensive fallbacks (e.g. for test doubles),
        // but StreamInterface is the actual case that must be handled.
        if (!($body instanceof StreamInterface) && !is_resource($body) && !is_string($body)) {
            throw FileStorageException::forOperation('download');
        }
